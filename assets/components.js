      ...
      <footer class="border-t border-slate-200 py-6 text-center text-sm text-slate-500 sm:py-8 dark:border-slate-800" role="contentinfo">
        <div class="shell">
          <div class="mb-4 flex flex-wrap justify-center gap-4 sm:gap-6 font-medium text-slate-600 dark:text-slate-300">
            <a href="https://wa.me/917492068998" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
          ...
          <div id="build-version" class="mt-4 inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-mono text-slate-500 dark:bg-slate-800 dark:text-slate-400" title="Latest deployment version">
            ...
            <span id="build-text">Build: 077d0a8 - just now  Master</span>
          </div>
        </div>
      </footer>
      `;

      // Fetch latest build version dynamically from local build.json
      fetch(basePath + 'assets/build.json?v=' + new Date().getTime())
        .then(r => r.json())
        .then(data => {
